...
def http_trigger(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if events_str_lst:
        events_json = json.dumps(events_str_lst)
        response_object =  process_events(events_json)

        return func.HttpResponse(response_object, mimetype="application/json", status_code = 200)
    else:
...
def process_events(events:list)->str:
    ...
    for event in events:
        ...
        if event_info == None:
            events_wrong_format_lst.append(event_info)
        else:
            events_info_lst.append(event_info)
    response_object = {
        "ok_list":events_info_lst,
        "bad_list":events_wrong_format_lst
    }
...
def extract_info_from_body(input:str):
    ...
    if any(x is None for x in (tech_name, mantis, client, service, cars_count)):
        ...
        data = {"tech_name":tech_name, "mantis": mantis, "client": client, "service": service}
